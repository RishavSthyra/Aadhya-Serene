import { isBackgroundTransitionActive } from './background-transition';
...
function isAmenitiesRoute(pathname) {
    return /^\/amenities(?:\/|$)/i.test(pathname ?? '');
}

function shouldPauseQueueJob(job) {
    if (typeof window === 'undefined' || !job) {
        return false;
    }

    if (job.pauseDuringBackgroundTransition && isBackgroundTransitionActive()) {
        return true;
    }

    if (job.pauseOnAmenitiesRoute && isAmenitiesRoute(window.location?.pathname)) {
        return true;
    }
...
function getNextRunnableJobIndex() {
    for (let index = 0; index < assetQueue.length; index += 1) {
        if (!shouldPauseQueueJob(assetQueue[index])) {
            return index;
        }
    }
...
function getNextPauseRetryMs() {
    let retryMs = Infinity;

    assetQueue.forEach((job) => {
        retryMs = Math.min(retryMs, job.pauseRetryMs ?? job.gapMs ?? DEFAULT_DELAY_MS);
    });

    return Number.isFinite(retryMs) ? retryMs : DEFAULT_DELAY_MS;
}
...
function pumpAssetQueue() {
    ...
    pumpScheduled = true;
    const nextJobIndex = getNextRunnableJobIndex();
    const nextJob = nextJobIndex >= 0 ? assetQueue[nextJobIndex] : assetQueue[0];
    schedulePrefetchWork(() => {
        ...
        const currentJobIndex = getNextRunnableJobIndex();
        if (currentJobIndex < 0) {
            window.setTimeout(pumpAssetQueue, getNextPauseRetryMs());
            return;
        }

        const currentJob = assetQueue[currentJobIndex];
        ...
        assetQueue.splice(currentJobIndex, 1);
        ...
        while (activeAssetFetches < concurrency && assetQueue.length > 0) {
            const parallelJobIndex = getNextRunnableJobIndex();
            if (parallelJobIndex < 0) {
                break;
            }

            const [parallelJob] = assetQueue.splice(parallelJobIndex, 1);
...
export function prefetchAssetsInChunks(urls, options = {}) {
    ...
    const {
        ...
        videoReadyEvent,
        timeoutMs,
        pauseDuringBackgroundTransition = false,
        pauseOnAmenitiesRoute = false,
        pauseRetryMs,
    } = options;
    ...
    uniqueUrls.forEach((url, index) => {
        ...
        assetQueue.push({
            ...
            videoReadyEvent,
            timeoutMs,
            pauseDuringBackgroundTransition,
            pauseOnAmenitiesRoute,
            pauseRetryMs,
        });
    });
