...
import { getNextLeadAssignment, getUnassignedLeadAssignment } from './lead-assignment';
...
function getHeaderValue(headers, key) {
    ...
    return cleanValue(headers.get(key));
}

export function getRequestMetadataFromHeaders(headers) {
...
export async function createEnquiryRecord(input) {
    ...
    const submissionWindow = cleanValue(input.submissionWindow);
    let assignment = getUnassignedLeadAssignment();

    try {
        assignment = await getNextLeadAssignment();
    } catch (error) {
        console.error('Unable to assign lead to a sales executive. Saving as unassigned.', error);
    }
    ...
    const record = await Notification.create({
        ...
        assignedSalesExecutiveId: assignment.assignedSalesExecutiveId,
        assignedSalesExecutiveName: assignment.assignedSalesExecutiveName,
        assignedSalesExecutiveEmail: assignment.assignedSalesExecutiveEmail,
...
export async function findLatestEnquiryRecord(query) {
    await connectMongo();

    return Notification.findOne(query).sort({ createdAt: -1 });
}

export async function findRecentEnquiryRecord({ phone, source, channel, minutes = 10 }) {
    await connectMongo();
...
export async function sendEnquiryNotificationEmail(submission) {
    ...
    const channelLabel =
        submission.channel === 'whatsapp_form'
            ? 'WhatsApp Lead'
            : submission.channel === 'portal_lead'
                ? 'Property Portal Lead'
                : 'Website Enquiry';
