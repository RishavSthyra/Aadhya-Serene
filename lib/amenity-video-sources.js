...
export const MANITEJA_R2_AMENITIES_BASE = 'https://cdn.sthyra.com/maniteja/videos/amenities';
...
const MANITEJA_R2_AMENITY_SLUGS = new Set([
    'indoorGames',
    'clubhouse',
]);
const AMENITY_LOWEST_QUALITY_OVERRIDES = Object.freeze({
    indoorGames: AMENITY_VIDEO_PREMIUM_QUALITY,
    clubhouse: AMENITY_VIDEO_PREMIUM_QUALITY,
});
...
export function getAmenityVideoBase(amenity) {
    if (MANITEJA_R2_AMENITY_SLUGS.has(amenity)) {
        return MANITEJA_R2_AMENITIES_BASE;
    }
...
function normalizeAmenityVideoQuality(amenity, quality) {
    if (quality !== AMENITY_VIDEO_QUALITY) {
        return quality;
    }

    return AMENITY_LOWEST_QUALITY_OVERRIDES[amenity] ?? quality;
}

export function getAmenityVideoSource(amenity, quality = AMENITY_VIDEO_QUALITY) {
    const normalizedQuality = normalizeAmenityVideoQuality(amenity, quality);
    return `${getAmenityVideoBase(amenity)}/${amenity}/${normalizedQuality}/${amenity}-h264.mp4`;
}
