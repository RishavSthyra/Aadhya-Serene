...
function getFlatVideoWarmupProfile(options = {}) {
    ...
    const isSlowConnection =
        saveData || effectiveType.includes('2g') || effectiveType.includes('slow');

    return {
        delayMs: options.delayMs ?? (isSlowConnection ? 1200 : 520),
        timeoutMs: options.timeoutMs ?? (isSlowConnection ? 1500 : 2200),
        idleTimeoutMs: options.idleTimeoutMs ?? (isSlowConnection ? 1400 : 900),
    };
}
...
function buildFlatVideoWarmupQueue(prioritizeFlatIds = []) {
    ...
    return orderedFlatIds.filter((flatId) => !warmedFlatVideoIds.has(flatId));
}
...
export function scheduleIdleFlatVideoWarmup(options = {}) {
    ...
    const queue = buildFlatVideoWarmupQueue(options.prioritizeFlatIds);
    if (!queue.length) {
        return () => {};
    }
