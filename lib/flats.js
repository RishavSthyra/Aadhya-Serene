...
/** Primary video: 1440p h264.mp4 (confirmed available on live CDN for all flats) */
export function flatVideoSrc(flatId, angle = 1) {
    const name = `flat_${flatId}_exterior_1-${angle}`;
    return `${CDN}/videos/flats/${name}/1440p/${name}-h264.mp4`;
}

/** High-quality webm: 2160p av1.webm (available for select flats) */
export function flatVideoMp4Src(flatId, angle = 1) {
    const name = `flat_${flatId}_exterior_1-${angle}`;
    return `${CDN}/videos/flats/${name}/2160p/${name}-av1.webm`;
}
...
export const WALKTHROUGH_VIDEO = `${CDN}/videos/homepage/3-1/1080p/3-1-h264.mp4`;
