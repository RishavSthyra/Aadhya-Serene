import {
    APARTMENT_360_FRAMES_PER_VIEW,
    apartment360FrameUrl,
    apartment360ViewIndexFromFrame,
} from './apartment360Frames';
...
const FLAT_RENDER_FALLBACK_FRAMES = Object.freeze({
    A1: apartment360FrameUrl(1),
    A2: apartment360FrameUrl(1 + APARTMENT_360_FRAMES_PER_VIEW),
    A3: apartment360FrameUrl(1 + (APARTMENT_360_FRAMES_PER_VIEW * 2)),
    A4: apartment360FrameUrl(1 + (APARTMENT_360_FRAMES_PER_VIEW * 3)),
});
...
const flatModelMap = {
    ...
    '405': { model: 18, status: 'available' }, '406': { model: 18, status: 'available' },
...
export function flatVideoFallbackId(flatId) {
    const normalizedFlatId = String(flatId ?? '').trim();
    return flatsWithVideo.has(normalizedFlatId) ? normalizedFlatId : null;
}
...
export function flatViewKeyFromFrame(frameNumber = 1) {
    return FLAT_RENDER_VIEW_KEYS[apartment360ViewIndexFromFrame(frameNumber)] ?? FLAT_RENDER_VIEW_KEYS[0];
}
...
export function getFlatById(flatId) {
    const normalizedFlatId = String(flatId ?? '').trim();
    return flatsData.find((f) => f.flat === normalizedFlatId);
}
