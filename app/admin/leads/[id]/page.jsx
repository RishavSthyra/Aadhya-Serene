...
function CallForm({ lead, canWrite, onSaved }) {
    ...
    async function submit(event) {
        ...
        try {
            ...
            onSaved(payload.callLog, payload.lifecycle);
    ...
    return (
        <main className="editorial-detail min-h-screen bg-[#f4f4f2] text-[#111]">
            <div className="mx-auto max-w-[1500px] px-5 py-7 sm:px-10 sm:py-10 lg:px-16 lg:py-12">
                ...
                {tab === 'calls' ? <section className="mt-8 max-w-4xl"><div className="flex items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#6b7280]">Conversation record</p><h2 className="mt-2 text-2xl font-bold">Calls</h2></div><span className="text-sm font-bold text-[#6b7280]">{lead.callLogs?.length || 0} total</span></div><div className="mt-7"><CallForm lead={lead} canWrite={canWrite} onSaved={(callLog, lifecycle) => { setLead((current) => ({ ...current, callLogs: [callLog, ...(current.callLogs || [])], lifecycle: lifecycle || current.lifecycle, callback: lifecycle?.callback || current.callback, stage: callLog.callOutcome === 'answered' ? 'answered' : 'not_answered', stageLabel: callLog.callOutcome === 'answered' ? 'Answered' : 'Not Answered' })); }} /></div><div className="mt-10 divide-y divide-[#111]/15">{(lead.callLogs || []).map((call) => <article key={call.id} className="py-5"><div className="flex items-start justify-between gap-4"><div><p className="font-bold">{formatDate(call.createdAt || call.callDate)}</p><p className="mt-1 text-sm text-[#4b5563]">{call.remark}</p>{call.answeredOutcomes?.length ? <p className="mt-2 text-xs font-bold text-[#6b7280]">{call.answeredOutcomes.map((item) => ANSWERED_CALL_OUTCOME_LABELS[item] || item).join(' · ')}</p> : null}</div><StatusBadge label={CALL_DISPOSITION_LABELS[call.callOutcome || call.callStatus] || call.callOutcome || 'Call'} /></div></article>)}</div></section> : null}
